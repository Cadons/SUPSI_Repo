...
void T2setInterruptPriority(int priority, int sub_priority) {//vector 8
    if (T2CONbits.T32 == 0) {
...
void init_Timer1(int T, unsigned long freq, int presc) {
    ...
    T1CONbits.ON = 0; //Disabilita timer
    //Imposta modalità timer

  
    switch (presc) {
        case 1:
            T1CONbits.TCKPS = 0b000; //prescalar 1
            presc = 1;
            break;
        case 8:
            T1CONbits.TCKPS = 0b011; //prescalar 8
            presc = 8;
            break;
       
        case 64:
            T1CONbits.TCKPS = 0b110; //prescalar 64
            presc = 64;
            break;
        case 256:
            T1CONbits.TCKPS = 0b111; //prescalar 256
            presc = 256;
            break;
        default:
            //se valore errato
    ...
    T1CONbits.TCS = 0;
    TMR1 = 0; //reset
    PR1 = (int) ((float) (T * pow(10, -3)) / (float) ((1 / (float) freq) * presc)); //calcola PR1 ossia valore massimo a cui il timer conta e poi ricomincia il ciclo

    macro_enable_interrupts();

        IEC0bits.T1IE = 1; // i n t e r r u p t Timer1 e n a b l e

}
void changePR1(int T, int freq, int presc)
{
        PR1 = (int) ((float) (T * pow(10, -3)) / (float) ((1 / (float) freq) * presc)); //calcola PR1 ossia valore massimo a cui il timer conta e poi ricomincia il ciclo

}
void T1setInterruptPriority(int priority, int sub_priority) {//vector 4
...
int awaitT1() {
    while (TMR1 <= (PR1 - 1)); //attendi che il timer raggiunga il valore di PR1-1
    return 1;
}