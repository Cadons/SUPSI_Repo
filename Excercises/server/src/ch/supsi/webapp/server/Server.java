...
public class Server {
     ...
     *
     */
    private static Content handleResponseContent(Request request) throws IOException {
        if (request.resource.equals("/prova.html")) {
            var output = Files.readAllBytes(Paths.get("C:\\Users\\Matte\\OneDrive - SUPSI\\A.A. 2022-2023\\Corsi\\SA\\Applicazioni Web 1\\Excercises\\server\\src\\ch\\supsi\\webapp\\server\\prova.html"));
            return new Content(new String(output).getBytes());
        } else if (request.resource.equals("/index.html")) {
         ...
         var output= new Content(("<!DOCTYPE html>\n" +
                    "<html>\n" +
                    "<head>\n" +
                    "<meta charset=\"UTF-8\">\n" +
    ...
    private static void produceResponse(OutputStream output, Content responseContent) throws IOException {
        ...
        if(!responseContent.isNotFound)
        output.write(String.valueOf(new Header("HTTP/1.1", 200, "OK", "text/html;charset=UTF-8", new String(responseContent.content).getBytes().length)).getBytes());
else
            output.write(String.valueOf(new Header("HTTP/1.1", 404, "OK", "text/html;charset=UTF-8", new String(responseContent.content).getBytes().length)).getBytes());
